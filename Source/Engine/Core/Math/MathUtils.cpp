...
namespace nc
{
    ...
    template<typename T>
    constexpr T SetBits(T a, T b) { return a | b; /*Use bitwise OR to set bits*/ }
    template<typename T>
    constexpr T ClearBits(T a, T b) { return a & (~b); /*Use bitwise AND with the complement to clear bits*/ }
    template<typename T>
    constexpr bool TestBits(T a, T b) { return (a & b) != 0; /*Use bitwise AND to test if bits are set*/ }
    template<typename T>
    constexpr T ToggleBits(T a, T b) { return a ^ b; /*Use bitwise XOR to toggle bits*/ }
}